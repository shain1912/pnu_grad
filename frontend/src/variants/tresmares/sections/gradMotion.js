 ...
 *   - useBarGrow      — bar-chart fills grow from 0 enter view
 ...
 * the NEXT card scrolls up to cover the previous, which dims + shrinks
 * recedes (monotonic bright → dark, never the reverse).
 */
export function useStickyStack(rootRef) {
  useEffect(() => {
    if (reduced() || !rootRef.current) return;
    gsap.registerPlugin(ScrollTrigger);
    const ctx = gsap.context(() => {
      const cards = gsap.utils.toArray(".tm-stack__card");
 ...
 * strokeDashoffset from full length → 0 chart scrolls through view. Dots
 * and labels (`[data-line-mark]`) fade in alongside the draw.
 */
export function useLineDraw(rootRef) {
  useEffect(() => {
    if (reduced() || !rootRef.current) return;
    gsap.registerPlugin(ScrollTrigger);
    const ctx = gsap.context(() => {
      const paths = gsap.utils.toArray("[data-line]");
...
/**
 * Count every `[data-count]` from 0 → its numeric target enters view.
...
/** Drift each `[data-ghost]` vertically page scrolls (parallax). */
export function useGhostParallax(rootRef) {
  useEffect(() => {
    if (reduced() || !rootRef.current) return;
    gsap.registerPlugin(ScrollTrigger);
    const ctx = gsap.context(() => {
      const ghosts = gsap.utils.toArray("[data-ghost]");
      ghosts.forEach((g) => {
...
/** Grow each `[data-bar]` fill from scaleX 0 → 1 enters view (data-viz). */
export function useBarGrow(rootRef) {
  useEffect(() => {
    if (reduced() || !rootRef.current) return;
    gsap.registerPlugin(ScrollTrigger);
    const ctx = gsap.context(() => {
